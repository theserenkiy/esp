...
void app_main(void)
{
    ...
    if (EXAMPLE_FATFS_MODE_READ_ONLY){
        err = esp_vfs_fat_spiflash_mount_ro(base_path, partname, &mount_config);
    } else {
        err = esp_vfs_fat_spiflash_mount_rw_wl(base_path, partname, &mount_config, &s_wl_handle);
    }
    ...
    if (EXAMPLE_FATFS_MODE_READ_ONLY){
        ESP_ERROR_CHECK(esp_vfs_fat_spiflash_unmount_ro(base_path, partname));
    } else {
